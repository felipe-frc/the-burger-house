...
public class MercadoPagoPreferenceServiceTests
{
    ...
    public async Task CreatesWithPaymentReferenceAndReusesById()
    {
        ...
        var service = new MercadoPagoPreferenceService(http, Options.Create(new MercadoPagoOptions
        {
            AccessToken = "local-test-token",
            ReturnUrl = "https://example.com/?checkout=return",
            NotificationUrl = "https://api.example.com/api/webhooks/mercadopago"
        }));
        var payment = new Payment(99, 43.90m, Guid.NewGuid().ToString("D"), PaymentMethod.Unknown);
        typeof(Payment).GetProperty(nameof(Payment.Id))!.SetValue(payment, 17);
        await service.GetOrCreateAsync(payment);
        Assert.Equal("pref-17", payment.ExternalPreferenceId);
        await service.GetOrCreateAsync(payment);
        Assert.Equal(new[] { "POST", "GET" }, handler.Methods);
        using var json = JsonDocument.Parse(handler.Body!);
        var root = json.RootElement;
        Assert.Equal("17", root.GetProperty("external_reference").GetString());
    ...
    public async Task RefusesUnpersistedPaymentWithoutHttp()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() => service.GetOrCreateAsync(new Payment(1, 1m, Guid.NewGuid().ToString("D"), PaymentMethod.Unknown)));
    ...
    {
        using var http = new HttpClient(new Stub());
        Assert.Throws<InvalidOperationException>(() => new MercadoPagoPreferenceService(http, Options.Create(new MercadoPagoOptions { AccessToken = "test", ReturnUrl = url })));
    }
    ...
    private sealed class Stub : HttpMessageHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken ct)
        {
            ...
            return new HttpResponseMessage(HttpStatusCode.OK) { Content = new StringContent("""{"id":"pref-17","external_reference":"17","init_point":"https://www.mercadopago.com.br/checkout/v1/redirect?pref_id=pref-17","expires":false}""") };
        }
    }
}
